...
<body>  
    ...
    <table>
        <tr>
            <td style="width:70%">Amount Php: {{ $profile->amountloan }}</td>
    ...
    <div>
        ...
        do hereby promise to pay ALLEN FINANCIAL SERVICES INCORPORATED or its order with business address at  Santa Filomena, Dipolog City, Philippines, the sum of <span class="underlyn big">{{ $profile->sum_in_words }}</span> (Php <span class="underlyn big">{{ $profile->amount_loan }}</span> ) Philippine Currency, subject to the following terms, amortization and installment due to wit. </p>
        <p>1. The above amount shall be payable within two (2) months from the date of the note or until the whole amount is fully paid with interest at the rate of  ten percent ( 10 % ) per month to be paid on daily installment at <span class="underlyn big">{{ $profile->per_day_in_words }}</span> (Php <span class="underlyn big">{{ number_format($ratePerDay,2) }}</span>) per day.<br>
